...
`platform` 只能是以下两个值之一（服务端白名单，与构建矩阵及下载页 `DOWNLOAD_PLATFORMS` 一致）：
...
服务端只受理这两个平台——构建矩阵只产出它们，发布导入脚本也只接受它们。客户端 `ClientUpdatePlatform` 类型仍保留历史值（`windows-arm` / `macos-x86` / `linux-x86`），但这些平台不在发布范围：查询会得到 `400`，客户端把检查失败静默处理为「无更新」。`windows-x86` 与 `macos-arm` 的 `release:latest:{platform}` 无记录时，客户端查询得到 `204`（无更新）；官网下载入口同样只覆盖这两个平台。
...
端点的主机名不需要单独配置：发布构建时 `scripts/write-tauri-release-config.mjs` 从 `VITE_LICENSE_SERVER_URL`（授权服务与更新服务共用的 ESA 域名）取出 origin，自动拼成上面的完整地址，并写入 `$RUNNER_TEMP` 下的临时 Tauri 配置。`{{target}}` 由客户端在 `check({ target })` 时替换，服务端只认上表两个值。同一变量也供前端授权服务使用，前端与 Rust 消费同一个域名。
...
| 字段 | 约束 | 客户端读取位置 |
| --- | --- | --- |
| `platform` | 必须是上述两个值之一，且与请求参数一致 | 不消费，仅服务端自校验与排障 |
| `version` | 裸 SemVer，不得附加 `v` 前缀（Rust 侧会 trim 前导 `v`，服务端也做归一化） | Rust 版本比较 + `DesktopUpdateInfo.version` |
| `url` | 公共 HTTPS 更新包地址（`macos-arm` 为 `*.app.tar.gz`，`windows-x86` 为签名 NSIS `.exe`）。必须是能被 `url::Url` 解析的绝对地址，且不含内嵌账号密码 | Rust `Update.download_url` |
...
发布工作流（`.github/workflows/build-desktop.yml`）把本次构建平台的安装包、更新包、签名与 `desktop-release-manifest.json` 一并上传到 GitHub Release。`v*` 标签的 Release 成功后，工作流会自动调用本仓库脚本导入 EdgeKV；以下命令用于本地手工补发：
...
- `check({ target, timeout: 15_000 })`，`target` 取 `ClientUpdatePlatform` 映射结果；映射到不在服务端白名单内的历史平台值时，服务端返回 `400`，检查失败由调用方静默处理为「无更新」；
