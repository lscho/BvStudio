...
本文档约束「动效展示」模块（资源面板 → 动效）中每个动效对免费用户的开放范围，与会员体系（见 `docs/17-会员授权服务协议.md`）配合。
...
- `CORE_FREE_EFFECT_IDS` 中的生成基础动效始终免费且不占用分类额度；当前包含仅图片 B-roll 所需的「静态图运镜」。
...
- 「展示」是**提升分类**（`PROMOTED_EFFECT_CATEGORIES`）：构造 `BUILTIN_EFFECTS` 时保留免费额度内的 10 个动效和不占额度的核心免费动效，其余整体改写为「高级」。因此展示分类下不会出现锁定条目。
