...
| `OSS_REGION` | Variable（可选） | Bucket 所在地域的 Region ID，如 `cn-hangzhou`（不带 `oss-` 前缀）。ossutil 2.x 使用 V4 签名时必填；配置了 `RELEASE_ASSET_BASE_URL` 时必填 |
...
| `ESA_KV_NAMESPACE` | Variable | 桌面发布记录所在的 ESA EdgeKV 命名空间，必须与线上边缘函数的 `KV_NAMESPACE` 一致；当前为 `bv_studio` |
| `ESA_ACCESS_KEY_ID` / `ESA_ACCESS_KEY_SECRET` | Secret | 自动将标签发布清单写入 ESA EdgeKV 的 RAM 凭证；仅 `v*` 标签发布会使用 |
