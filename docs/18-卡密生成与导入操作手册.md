...
BVideo Pro 会员卡密的全套操作命令。背景与架构见 [`17-会员授权服务协议.md`](17-会员授权服务协议.md)；本文只讲操作。
...
> 命名空间同时存放桌面更新发布记录 `release:latest:{platform}`（见 [`16-桌面更新发布操作手册.md`](16-桌面更新发布操作手册.md)）。不带 `--prefix` 的全量清理会一并删除这些键，客户端随后收到 204 不再提示更新，需要重新执行 `npm run esa:import-release` 才能恢复。
