...
5. **产物托管（可选，但国内发布强烈建议）**：把安装包/更新包放到阿里云对象存储 + CDN，客户端更新与官网下载页都会指向它。
   ...
   CI 会自动拼出 `{基址}/{应用名}/releases/v{版本}`，并把五平台产物上传到 OSS 的**同名路径**下。应用名取自 `package.json` 的 `name`（`bframe-studio`）——下载域名可能同时托管多个应用，靠这一层隔离；不用 `tauri.conf.json` 的 `productName`，因为它含空格，进 URL 会被编码成 `%20`。
...
#    （五平台构建 + 签名 → 生成清单 → 上传对象存储 → 发布清单 artifact）
