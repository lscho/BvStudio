...
同一个边缘函数和命名空间还承载桌面更新服务（`GET /api/desktop-updates/latest`，键 `release:latest:{platform}`），入口在 `edge/index.js` 按精确路径分发，两条业务互不影响；协议见 [`15-桌面更新服务协议.md`](15-桌面更新服务协议.md)。
...
- 卡密级自愈：verify 时回读设备记录指向的卡密，发现 `revoked` 或已改绑其他设备，即清除设备记录并返回 Free。刚兑换后的 60 秒内若卡密回读仍是缺失或 `unused`，视为 EdgeKV 尚未传播并暂时信任新设备记录，避免兑换成功后立即回退 Free；明确吊销或改绑不受该宽限影响。运营侧只需在 EdgeKV 控制台改 `card:*` 的 `status` 即可吊销。
...
| 变量 | 说明 |
| --- | --- |
| `VITE_LICENSE_SERVER_URL` | 边缘函数绑定的 API 域名，**只填 origin**（如 `https://license.example.com`，不带路径、查询串或结尾斜杠）。桌面更新端点也由它派生，见 [`15-桌面更新服务协议.md`](15-桌面更新服务协议.md) |
...
完整命令与参数见 [`18-卡密生成与导入操作手册.md`](18-卡密生成与导入操作手册.md)。速查：
