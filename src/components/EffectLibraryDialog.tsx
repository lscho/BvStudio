    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content effect-library-dialog" aria-describedby="effect-library-description">
          ...
          <Dialog.Description id="effect-library-description">管理本机安装的声明式动效包。</Dialog.Description>
          ...
          {pendingRemoval && <div className="remove-warning"><Trash2 size={18} /><span><strong>卸载 {pendingRemoval.manifest.name}？</strong><small>它会从动效库移除；现有工程仍使用已经保存的 recipe 快照。</small></span><div className="confirm-actions"><button type="button" disabled={busy} onClick={() => setPendingRemoval(null)}>取消</button><button className="button secondary danger-action" type="button" disabled={busy} onClick={() => void removePackage(pendingRemoval.manifest.id)}>确认卸载</button></div></div>}
          <div className="package-list">{packages.length ? packages.map((item) => <div key={item.manifest.id}><PackageOpen size={18} /><span><strong>{item.manifest.name}</strong><small>{item.manifest.author} · {item.manifest.version} · {item.effects.length} 个动效</small></span>{item.verified && <BadgeCheck className="verified" size={16} />}<button type="button" aria-label={`卸载 ${item.manifest.name}`} title="卸载动效包" disabled={busy} onClick={() => setPendingRemoval(item)}><Trash2 size={14} /></button></div>) : <p className="empty-copy">尚未安装扩展动效包</p>}</div>
