...
import { AudioLines, AudioWaveform, BadgePercent, Captions, ChartNoAxesColumnIncreasing, ChartPie, ChartSpline, ChevronDown, Download, FileVideo2, ImageIcon, Layers3, Link2, Music2, PackageOpen, Plus, SlidersHorizontal, Sparkles, TriangleAlert, WandSparkles } from "lucide-react";
...
export function AssetPanel({ onImport, onGenerate, onMatchEffects, onTranscribe, onExtractAudio, onExportAudio, onRelink, onCreateAudio, onManageEffects }: Props) {
  ...
  const effectGroups = useMemo(() => (["场景", "标题", "强调", "卡片", "标注", "数据", "布局"] satisfies EffectCategory[])
    ...
    .filter((group) => group.effects.length > 0), [effects]);
  return (
      ...
      <Tabs.Root defaultValue={assets.length ? "media" : "effects"} className="panel-tabs">
        ...
        <Tabs.Content value="subtitles" className="panel-content subtitles-panel" tabIndex={-1}>
          ...
          <div className="subtitle-actions" aria-label="字幕操作">
            ...
            <button type="button" title="自动识别连续字幕并匹配单条或组合动效" disabled={!subtitles.length} onClick={onMatchEffects}><WandSparkles size={15} />匹配</button>
...
function EffectThumbnail({ effect }: { effect: EffectDefinition }) {
  ...
  return (
    <span
      ...
      aria-hidden="true"
    >
      <i>{effect.recipe.sceneBackground ? <Layers3 size={18} /> : chartIcon ?? label}</i>
    </span>
  );
}
