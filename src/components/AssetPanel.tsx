...
import { AudioLines, AudioWaveform, BadgePercent, Captions, ChartNoAxesColumnIncreasing, ChartPie, ChartSpline, Check, ChevronDown, Download, FileVideo2, ImageIcon, Layers3, Link2, Music2, PackageOpen, Play, Plus, Search, SlidersHorizontal, Sparkles, TriangleAlert, WandSparkles, X } from "lucide-react";
...
export function AssetPanel({ onImport, onGenerate, onMatchEffects, onTranscribe, onExtractAudio, onExportAudio, onRelink, onCreateAudio, onManageEffects, onPreviewBuiltinSound, onAddBuiltinSound }: Props) {
      ...
      <Tabs.Root defaultValue={assets.length ? "media" : "effects"} className="panel-tabs">
            ...
            {effectGroups.map((group) => (
              <details key={group.category} className="effect-group" open>
                ...
                <div className="effect-group-items">
                  {group.effects.map((effect) => <button type="button" key={effect.id} onClick={() => addEffect(effect.id)}><EffectThumbnail effect={effect} accentColor={motionAccentColor} /><span><strong>{effect.name}</strong><small>{effect.description}</small></span><Plus size={15} /></button>)}
                </div>
              </details>
            ))}
          </div> : <p className="empty-copy align-left">没有匹配的动效</p>}
        </Tabs.Content>
        <Tabs.Content value="sounds" className="panel-content sound-library" tabIndex={-1}>
        ...
        <Tabs.Content value="subtitles" className="panel-content subtitles-panel" tabIndex={-1}>
          ...
          <div className="subtitle-actions" aria-label="字幕操作">
            ...
            <button type="button" title="按连续场景自动匹配 A-roll、B-roll、多层动效与音效" disabled={!subtitles.length} onClick={onMatchEffects}><WandSparkles size={15} />匹配</button>
...
function EffectThumbnail({ effect, accentColor }: { effect: EffectDefinition; accentColor: string }) {
