...
export function Timeline() {
  ...
  function snapTime(timeUs: number, excludedIds: string[]): number {
    ...
    const boundaries = [0, playheadUs, ...allClips.filter((clip) => !excludedIds.includes(clip.id)).flatMap((clip) => [clip.startUs, clipEnd(clip)])];
  ...
  function seek(event: React.MouseEvent<HTMLDivElement>) {
    if ((event.target as HTMLElement).closest(".timeline-clip, .track-control")) return;
