...
it("提交纯模式和分镜要求，冲突要求留在本地", async () => {
  ...
  expect(generate).toHaveBeenCalledWith(expect.objectContaining({ storyboard: { mode: "a-roll", prompt: "0-5秒 A-roll 保留口播" }, soundEnabled: true, beatSync: false }), expect.any(AbortSignal), expect.any(Function));
  expect(apply).not.toHaveBeenCalled();
  fireEvent.click(screen.getByRole("button", { name: "应用整套编排" }));
  expect(apply).toHaveBeenCalledWith(preview, expect.objectContaining({ storyboard: { mode: "a-roll", prompt: "0-5秒 A-roll 保留口播" } }));
});
