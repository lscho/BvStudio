...
export function CanvasSettingsDialog({ open, onOpenChange, canvas, assets }: Props) {
  ...
  const [fps, setFps] = useState(canvas.fpsNumerator / canvas.fpsDenominator);
  ...
  useEffect(() => {
    ...
    setFps(canvas.fpsNumerator / canvas.fpsDenominator);
  }, [canvas, open]);
    ...
    <Dialog.Portal>
      ...
      <Dialog.Content className="dialog-content canvas-dialog" aria-describedby="canvas-description">
        ...
        <form className="settings-form" onSubmit={submit}>
          ...
          {sourceSizes.length > 0 && <label><span>跟随素材</span><select value="" onChange={(event) => { const source = sourceSizes.find((item) => item.id === event.target.value); if (source) choose(source.width, source.height, source.fps); }}><option value="">选择已导入视频规格</option>{sourceSizes.map((source) => <option key={source.id} value={source.id}>{source.name} · {source.width} × {source.height}</option>)}</select></label>}
          ...
          <label><span>帧率</span><select value={fps} onChange={(event) => setFps(Number(event.target.value))}><option value={23.976}>23.976 fps</option><option value={24}>24 fps</option><option value={25}>25 fps</option><option value={29.97}>29.97 fps</option><option value={30}>30 fps</option><option value={50}>50 fps</option><option value={59.94}>59.94 fps</option><option value={60}>60 fps</option></select></label>
