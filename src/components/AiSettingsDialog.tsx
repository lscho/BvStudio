...
import { Check, Cloud, Cpu, Gauge, KeyRound, LoaderCircle, PlugZap, X } from "lucide-react";
...
export function AiSettingsDialog({ open, settings, onOpenChange, onSave }: Props) {
  const [section, setSection] = useState<"provider" | "media" | "asr">("provider");
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content settings-dialog" aria-describedby="ai-settings-description">
          ...
          <div className="settings-dialog-header">
            ...
            <Dialog.Description id="ai-settings-description">模型、媒体与本地字幕能力统一保存在当前客户端。</Dialog.Description>
          </div>
          <form className="settings-form" onSubmit={submit}>
            <div className="settings-layout">
              <nav className="settings-nav" aria-label="设置分组">
                ...
                <button type="button" className={section === "asr" ? "active" : ""} onClick={() => setSection("asr")}><Cpu size={17} /><span><strong>本地字幕</strong><small>Qwen3-ASR</small></span></button>
              </nav>
              <section className="settings-pane">
                {section === "provider" && <div className="settings-section-panel">
                  <div className="settings-pane-heading"><h3>云端大模型</h3><p>兼容 OpenAI 与 Anthropic 协议，API Key 在桌面端写入系统钥匙串。</p></div>
                ...
                {section === "asr" && <div className="settings-section-panel">
                  <div className="settings-pane-heading"><h3>本地字幕模型</h3><p>Qwen3-ASR 在本机运行，音视频不会上传。</p></div>
