...
import {
  ...
  TypeShiftCard,
  UICalloutCard
} from "@/compositions/overlayStudioCards";

export type CompositionControl =
...
export interface ReactEffectDefinition {
  ...
  controls: readonly CompositionControl[];
  motionDurationUs: number;
}

const commonControls: readonly CompositionControl[] = [
...
const structuredTextControls: readonly CompositionControl[] = commonControls.map((control) => (
...
const themeControl: CompositionControl = { kind: "param-select", field: "theme", label: "卡片底色", options: [{ value: "dark", label: "暗底" }, { value: "light", label: "亮底" }] };
const paramControls = (...controls: CompositionControl[]): readonly CompositionControl[] => [themeControl, ...controls, ...appearanceControls];
...
const componentRegistrations: Readonly<Record<string, ComponentRegistration>> = {
  ...
  "focus-card": { component: FocusCard, controls: paramControls(
    ...
    { kind: "param-range", field: "moveMs", label: "人物入场时长", min: 200, max: 2000, step: 20, suffix: "ms" },
...
function GenericEffectCard(props: CompositionRenderProps) {
...
export function reactEffectDefinition(compositionId: string): ReactEffectDefinition {
  const registration = componentRegistrations[compositionId];
  return {
    definition: compositionById(compositionId),
...
export function activeReactEffectDefinitions(): ReactEffectDefinition[] {
  return allCompositions().filter((definition) => definition.renderer !== "three" && definition.renderer !== "canvas").map((definition) => reactEffectDefinition(definition.id));
}
...
export function effectControlsFor(clip: CompositionClip): readonly CompositionControl[] {
  return reactEffectDefinition(clip.compositionId).controls.map((control) => (
    control.kind === "text" && clip.recipe?.chart ? { ...control, label: "说明文字（可留空）" } : control
  ));
}

export function reactEffectMotionDurationUs(compositionId: string) {
  return reactEffectDefinition(compositionId).motionDurationUs;
}
