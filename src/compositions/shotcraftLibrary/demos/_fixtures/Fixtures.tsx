...
import { useShotcraftContent } from '@/compositions/shotcraftLibrary/runtime';
...
const PlaceholderDashboard: React.FC<{ variant?: 'A' | 'B' }> = ({ variant = 'A' }) => (
...
export const Card: React.FC<{ w: number; h: number; seed?: number; style?: React.CSSProperties }> = (props) => {
  const content = useShotcraftContent();
  if (content.linked) return <div style={{ width: props.w, height: props.h, overflow: "hidden", background: "#111316", ...props.style }}><div style={{ position: "relative", width: 1920, height: 1080, transformOrigin: "0 0", transform: `scale(${props.w / 1920}, ${props.h / 1080})` }}>{content.incoming}</div></div>;
  const url = content.images.length ? content.images[Math.abs(props.seed ?? 0) % content.images.length] : undefined;
  if (!url) return <PlaceholderCard {...props} />;
  return <div style={{ width: props.w, height: props.h, overflow: "hidden", borderRadius: 14, ...props.style }}><img src={url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div>;
};
export const FakeDashboard: React.FC<{ variant?: "A" | "B" }> = ({ variant = "A" }) => {
  const content = useShotcraftContent();
  const scene = variant === "A" ? content.outgoing : content.incoming;
  if (scene) return <div style={{ position: "relative", width: 1920, height: 1080 }}>{scene}</div>;
  const url = content.images[variant === "A" ? 0 : 1] ?? content.images[0];
  return url ? <img src={url} alt="" style={{ width: 1920, height: 1080, objectFit: "contain" }} /> : <PlaceholderDashboard variant={variant} />;
};
