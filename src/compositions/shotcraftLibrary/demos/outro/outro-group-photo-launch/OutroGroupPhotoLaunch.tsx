...
import { PageCam2D } from '@/compositions/shotcraftLibrary/demos/_fixtures/PageCam2D';
import layout from '@/compositions/shotcraftLibrary/demos/_textures/live-layout.json';
import { contentText, useShotcraftContent, shotcraftImageUrl, type ShotcraftContent } from "@/compositions/shotcraftLibrary/runtime";

export function createDemo(content: ShotcraftContent) {
const OUTRO_GROUP_PHOTO_LAUNCH_DURATION = 145;
...
const OutroGroupPhotoLaunch: React.FC = () => {
  const frame = useCurrentFrame();
  const media = useShotcraftContent();
        ...
        <AbsoluteFill style={{ pointerEvents: 'none' }}>
          {ELS.map((el) => {
            ...
            const texture = el.wbrCrop
              ? {
                  background: `#fff url("${shotcraftImageUrl(media, `textures/live/${el.file}`)}") -576px -173px / 1920px ${WBR_PAGE_H}px no-repeat`,
      ...
      <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', pointerEvents: 'none' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: MONO, fontSize: 25, letterSpacing: '0.14em', color: 'oklch(50% 0.006 82)', marginTop: 30, opacity: tag, textTransform: 'uppercase' }}>
            {contentText(content, "copy0", "Team Research Console")}</div>
        </div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
};
return OutroGroupPhotoLaunch;
}
