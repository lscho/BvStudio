...
const Panel: React.FC<{ shade: number; t?: number }> = ({ shade, t = 1 }) => (
  <div style={{
    ...
    <div style={{
      ...
      <FloatWrap h={liftOf(t, 0.3)}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <div style={{ fontSize: 66, color: INK, fontWeight: 500 }}>{contentText(content, "copy0", "Product analytics")}</div>
        </div>
      </FloatWrap>
      <FloatWrap h={liftOf(t, 0.42, 280)}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ width: 72, height: 72, borderRadius: 20, background: '#4147f5', marginLeft: 150, flexShrink: 0 }} />
          <div style={{ fontSize: 165, color: '#26262c', fontWeight: 550, marginLeft: 110, letterSpacing: 1, whiteSpace: 'nowrap' }}>{contentText(content, "copy1", "ClickUp 3.0")}</div>
        </div>
      </FloatWrap>
    ...
    <div style={{ position: 'absolute', left: 120, top: 560 }}>
      <FloatWrap h={liftOf(t, 0.54, 250)}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
          <CULogo size={104} />
          <div style={{ fontSize: 92, fontWeight: 800, color: '#222228', letterSpacing: -1 }}>{contentText(content, "copy2", "ClickUp")}</div>
        </div>
      </FloatWrap>
    </div>
    ...
    <div style={{ position: 'absolute', left: 110, top: 830, width: 2900 }}>
      <FloatWrap h={liftOf(t, 0.68, 260)}>
        <div style={{
          ...
          <div style={{ fontSize: 70, color: '#5353cf', fontWeight: 550 }}>{contentText(content, "copy3", "Home")}</div>
        </div>
      </FloatWrap>
    </div>
    {/* Inbox 行 */}
    <div style={{ position: 'absolute', left: 166, top: 1070 }}>
      <FloatWrap h={liftOf(t, 0.8, 240)}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 40 }}>
          ...
          <div style={{ fontSize: 70, color: INK }}>{contentText(content, "copy4", "Inbox")}</div>
        </div>
      </FloatWrap>
    </div>
    {/* 更多侧栏行 */}
    ...
    <div style={{ position: 'absolute', left: 3050, top: 300, fontSize: 430, fontWeight: 700, color: '#26262c', letterSpacing: -6 }}>{contentText(content, "copy5", "W")}</div>
    {/* 主区白色圆角卡群（右下淡白砖） */}
    ...
    <div style={{
      ...
      <div style={{ position: 'absolute', left: 220, top: 200, fontSize: 150, fontWeight: 750, color: '#232329', letterSpacing: -2, whiteSpace: 'nowrap' }}>{contentText(content, "copy6", "Product Management")}</div>
      {/* List / Board tabs */}
      <div style={{ position: 'absolute', left: 240, top: 480, display: 'flex', gap: 110, alignItems: 'center' }}>
        <div style={{
          ...
          <div style={{ fontSize: 62, fontWeight: 600, color: '#2c2c33' }}>{contentText(content, "copy7", "List")}</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
          <svg width={56} height={56} viewBox="0 0 40 40">
          ...
          <div style={{ fontSize: 62, fontWeight: 500, color: '#2c2c33' }}>{contentText(content, "copy8", "Board")}</div>
        </div>
        <div style={{ fontSize: 56, color: '#9a9aa4', marginLeft: -40 }}>{contentText(content, "copy9", "11")}</div>
      </div>
      ...
      }}>{contentText(content, "copy10", "IN PROGRESS")}</div>
      <div style={{ position: 'absolute', left: 244, top: 1030, fontSize: 44, fontWeight: 600, color: '#8f8f98', letterSpacing: 1 }}>{contentText(content, "copy11", "TASK NAME")}</div>
      {/* 任务行 */}
