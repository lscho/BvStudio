...
const CAPSULES = [
  { text: contentText(content, "copy0", "Hallo!"), idx: 5 },
  { text: contentText(content, "copy1", "¡Hola!"), idx: 14 },
  { text: contentText(content, "copy2", "Ciao!"), idx: 23 },
];
...
const BubbleSwarmTakeover: React.FC = () => {
  ...
  const breathe = content.linked ? 1 : 1 + 0.02 * Math.sin(frame * 0.045);
  return (
    <AbsoluteFill style={{ background: '#ececea', overflow: 'hidden' }}>
      ...
      {/* i18n 文字胶囊混在中层气泡里漂 */}
      {!content.linked && CAPSULES.map((c, i) => {
