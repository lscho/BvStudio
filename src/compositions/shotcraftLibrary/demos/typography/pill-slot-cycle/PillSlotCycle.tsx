...
const PILLS = [
  { label: contentText(content, "copy0", "Ask a question"), icon: '?' },
  { label: contentText(content, "copy1", "Find in Drive"), icon: '▲' },
  { label: contentText(content, "copy2", "Find in Slack"), icon: '#' },
  { label: contentText(content, "copy3", "Summarize"), icon: '≡' },
  { label: contentText(content, "copy4", "Improve writing"), icon: '✎' },
  { label: contentText(content, "copy5", "Draft an agenda"), icon: '☰' },
];
const BEAT = 21;
const INTRO = 12;
  ...
  if (!isFinale && rel >= 0) {
    ...
    slot = (
      <div style={{ position: 'relative', display: 'inline-block' }}>
        ...
        >
          {contentText(content, "copy6", "do it all.")}</span>
        {lastOutT < 1 && (
          <div
            style={{
              position: 'absolute',
  ...
  return (
    <AbsoluteFill style={{ background: G.bg }}>
      ...
      >
        ...
        >
          {contentText(content, "copy7", "One AI tool to")}</span>
        {slot}
      </div>
    </AbsoluteFill>
  );
};
return PillSlotCycle;
}
