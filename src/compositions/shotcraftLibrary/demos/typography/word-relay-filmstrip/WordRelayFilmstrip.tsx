...
const WORDS = [contentText(content, "copy0", "researches"), contentText(content, "copy1", "builds"), contentText(content, "copy2", "codes")];
const SWITCHES = [14, 62, 108];
const SW_DUR = 16;
const SERIF = '"Didot", "Bodoni 72", "Playfair Display", Georgia, serif';
const WordRelayFilmstrip: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: '#faf8f3' }}>
      ...
      <div style={{ position: 'absolute', right: 210, top: 402 }}>
        <div style={{ ...wordStyle, color: '#191919' }}>{contentText(content, "copy3", "Computer")}</div>
        <div style={{ position: 'relative', height: 140 }}>{wordNodes}</div>
      </div>
    </AbsoluteFill>
  );
};
return WordRelayFilmstrip;
}
