...
const WORDS: { text: string; mr: number; win: [number, number] }[] = [
  { text: contentText(content, "copy0", "min"), mr: 11, win: [0.54, 0.68] },
  { text: contentText(content, "copy1", "to"), mr: 11, win: [0.62, 0.78] },
  { text: contentText(content, "copy2", "install"), mr: 0, win: [0.7, 0.9] },
];
const mix = (k: number, a: number, b: number) => Math.round(lerp(k, a, b));
const CountdownArcScatter: React.FC = () => {
  const t = useT();
