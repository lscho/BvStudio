...
const TitleDemoteToLabel: React.FC = () => {
  const frame = useCurrentFrame();
  const SPLIT = 92; // 变体 A 时长

  if (frame < SPLIT) {
    return <DemoteScene frame={frame} title={contentText(content, "copy0", "Running Subagents")} withSelection={false} />;
  }
  // 变体 B：文本选中态高亮登场
  const f = frame - SPLIT;
  // 白闪转场 3f
  ...
  return (
    <AbsoluteFill>
      <DemoteScene frame={f} title={contentText(content, "copy1", "Select the Answer")} withSelection={true} />
      <AbsoluteFill style={{ background: '#fff', opacity: flash, pointerEvents: 'none' }} />
    </AbsoluteFill>
  );
};
return TitleDemoteToLabel;
}
