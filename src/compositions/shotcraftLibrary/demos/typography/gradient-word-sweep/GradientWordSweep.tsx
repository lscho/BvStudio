...
const GradientWordSweep: React.FC = () => {
      ...
      >
        <div style={lineStyle}>
          <span style={{ position: 'relative', display: 'inline-block' }}>
            <span>{contentText(content, "copy0", "Supercharged")}</span>
            {/* AE 式辉光：大半径柔光层（最糊） */}
            <span
              aria-hidden
              style={{
            ...
            >
              {contentText(content, "copy0", "Supercharged")}</span>
            {/* 中晕层 */}
            <span
              aria-hidden
              style={{
          ...
          <span>{contentText(content, "copy1", "performance")}</span>
        </div>
        <div style={lineStyle}>{contentText(content, "copy2", "with rock-solid reliability")}</div>
      </div>
    </AbsoluteFill>
  );
};
return GradientWordSweep;
}
