...
const MarkerUnderlineTitle: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: '#f4f4f2', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{
        ...
        <div>
          {contentText(content, "copy0", "Meet the")}{' '}
          <span style={{ fontStyle: 'italic', position: 'relative', display: 'inline-block' }}>
            {contentText(content, "copy1", "new")}{/* 马克笔下划线：clip 从左到右揭示，保留笔形本身的粗细变化 */}
            <svg
        ...
        <div>{contentText(content, "copy2", "Notion AI")}</div>
      </div>
    </AbsoluteFill>
  );
};
return MarkerUnderlineTitle;
}
