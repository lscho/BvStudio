...
const OscilloscopeStreamV2: React.FC = () => {
  ...
  return (
    <div style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 110, width: '100%', textAlign: 'center' }}>
        <TitleBlock text={contentText(content, "copy0", "OSCILLOSCOPE STREAM V2")} size={72} />
      </div>
      ...
      >
        ...
        <div style={{ position: 'absolute', left: PAD, top: 38, fontFamily: 'Helvetica, Arial, sans-serif' }}>
          <div style={{ fontSize: 30, fontWeight: 700, color: G.ink }}>{contentText(content, "copy1", "Requests per second")}</div>
          <div style={{ fontSize: 20, fontWeight: 500, color: G.mid, marginTop: 8 }}>{contentText(content, "copy2", "api-gateway · production · last 60 s")}</div>
        </div>
        <div
          style={{
            position: 'absolute',
        ...
        >
          ...
          <div style={{ fontSize: 18, fontWeight: 600, color: hot ? AMBER : G.mid, marginTop: 2 }}>{contentText(content, "copy3", "req/s · live")}</div>
        </div>
