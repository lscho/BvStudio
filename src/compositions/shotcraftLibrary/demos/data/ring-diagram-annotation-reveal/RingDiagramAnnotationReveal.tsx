...
import { Easing, interpolate, useCurrentFrame } from "@/compositions/shotcraftLibrary/runtime";
import { DesignStage } from '@/compositions/shotcraftLibrary/demos/_fixtures/Motion';
...
const RingDiagramAnnotationReveal: React.FC = () => {
    ...
    <DesignStage bg="#f7f7f5" raster="zoom">
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', fontFamily: 'Arial, sans-serif' }}>
        ...
        >
          ...
          >
            {contentText(content, "copy0", "CONTENT")}</div>
          <div
            style={{
              position: 'absolute',
              left: 79,
          ...
          >
            {contentText(content, "copy1", "SUBJECT")}</div>
        </div>
        ...
        >
          {contentText(content, "copy2", "EXPLANATION")}</div>
        <div
          style={{
            position: 'absolute',
            left: 325,
        ...
        >
          {contentText(content, "copy3", "SUPPORTING DETAIL")}</div>
      </div>
    </DesignStage>
  );
};
return RingDiagramAnnotationReveal;
}
