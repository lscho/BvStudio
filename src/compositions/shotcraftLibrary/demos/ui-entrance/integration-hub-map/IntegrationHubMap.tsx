...
const LIST: { icon: string; title: string; sub: string }[] = [
  { icon: '#4a9fd8', title: contentText(content, "copy0", "Q3 Enterprise Deal"), sub: contentText(content, "copy1", "Revenue · Pipeline · Q3 Quota") },
  { icon: '#4a9fd8', title: contentText(content, "copy2", "Major Enterprise Account - UK"), sub: contentText(content, "copy3", "Revenue · MQL · International") },
  { icon: '#34a853', title: contentText(content, "copy4", "Enterprise Pitch Deck"), sub: contentText(content, "copy5", "Open in GDrive") },
  { icon: '#a259ff', title: contentText(content, "copy6", "MQL Lead Form Design"), sub: contentText(content, "copy7", "Figma File · Last Edited") },
  { icon: '#f2c744', title: contentText(content, "copy8", "Enterprise Sales"), sub: contentText(content, "copy9", "ClickUp Space") },
  { icon: '#9a9a98', title: contentText(content, "copy10", "Enterprise Closed Archive"), sub: contentText(content, "copy11", "Archived · In Enterprise Sales") },
  { icon: '#c8c8c6', title: contentText(content, "copy12", "Open Enterprise Lead - Follow up"), sub: contentText(content, "copy13", "In Progress · In Enterprise Sales · Yesterday") },
];
const HubPanel: React.FC<{ glow: number }> = ({ glow }) => (
  ...
  >
    <div style={{ flex: 2 }}>
      <div style={{ fontSize: 27, fontWeight: 600, color: '#2f2f38' }}>{contentText(content, "copy14", "Enterprise MQLs")}</div>
      ...
      <div style={{ fontSize: 12, color: '#8b8b92', marginTop: 14 }}>{contentText(content, "copy15", "Recent")}</div>
      {LIST.map((it, i) => (
    ...
    <div style={{ flex: 1, borderLeft: '1px solid #e7e7e5', paddingLeft: 22 }}>
      ...
      >
        {contentText(content, "copy16", "+ Add Location Filter")}</div>
      <div style={{ fontSize: 10.5, color: '#adadb2', marginTop: 24, letterSpacing: 1 }}>{contentText(content, "copy17", "QUICK FILTERS")}</div>
      {['Assigned to Me', 'Created by Me'].map((t) => (
        <div key={t} style={{ fontSize: 13.5, color: '#55555c', marginTop: 11 }}>{t}</div>
      ))}
      <div style={{ fontSize: 10.5, color: '#adadb2', marginTop: 24, letterSpacing: 1 }}>{contentText(content, "copy18", "TASK FILTERS")}</div>
...
const FrontPanel: React.FC<{ glow: number }> = ({ glow }) => (
  ...
  >
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ fontSize: 26, fontWeight: 600, color: '#2f2f38' }}>{contentText(content, "copy0", "Q3 Enterprise Deal")}</div>
    </div>
...
const IntegrationHubMap: React.FC = () => {
