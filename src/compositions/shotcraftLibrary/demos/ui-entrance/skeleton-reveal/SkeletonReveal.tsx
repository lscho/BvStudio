...
const NAMES = [contentText(content, "copy0", "Ana"), contentText(content, "copy1", "Ben"), contentText(content, "copy2", "Kai"), contentText(content, "copy3", "Mia")];
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', gap: 22, opacity: dev > 0.02 ? 1 : 0 }}>
        ...
        <div style={{ flex: 1, paddingTop: 2 }}>
          <div style={{ fontSize: 26, fontWeight: 800, color: INK, opacity: dev }}>
            {NAMES[i]}
            <span style={{ fontWeight: 400, fontSize: 19, color: '#9a9a98', marginLeft: 12 }}>{contentText(content, "copy4", "9:0")}{i + 1} {contentText(content, "copy5", "AM")}</span>
          </div>
