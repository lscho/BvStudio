...
const CARDS: { title: string; sub: string; col: number; row: number }[] = [
  { title: contentText(content, "copy0", "Creative Refresh"), sub: contentText(content, "copy1", "New logo exploration"), col: 0, row: 0 },
  { title: contentText(content, "copy2", "New Bugs Per Week"), sub: contentText(content, "copy3", "Bug tracker Dashboard"), col: 1, row: 0 },
  { title: contentText(content, "copy4", "Tiger Team Roadmap"), sub: contentText(content, "copy5", "Roadmap Outline"), col: 2, row: 0 },
  { title: contentText(content, "copy6", "Design System"), sub: contentText(content, "copy7", "Design Handbook Inspo"), col: 3, row: 0 },
  { title: contentText(content, "copy8", "Development Sprint Dashboard"), sub: contentText(content, "copy9", "Dev Team Sprints"), col: 0, row: 1 },
  { title: contentText(content, "copy10", "CSS Bug Tracker"), sub: contentText(content, "copy11", "Query Reports"), col: 1, row: 1 },
  { title: contentText(content, "copy12", "Platform"), sub: contentText(content, "copy13", "System Health Monitor"), col: 2, row: 1 },
];
    ...
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      <div style={{ width: 860, borderRight: `4px solid ${FAINT}`, padding: '70px 60px 0', background: '#f1f1f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
          ...
          <div style={{ fontFamily: FONT, fontSize: 56, fontWeight: 800, color: INK }}>{contentText(content, "copy14", "ClickUp")}</div>
        </div>
        <div style={{ height: 46 }} />
        ...
        <div style={{ height: 60 }} />
        <div style={{ fontFamily: FONT, fontSize: 38, letterSpacing: 5, color: MID, fontWeight: 600, paddingLeft: 32 }}>{contentText(content, "copy15", "SPACES")}</div>
        <div style={{ height: 16 }} />
      ...
      <div style={{ flex: 1, padding: '70px 100px 0', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: 110, fontFamily: FONT, fontSize: 42, color: MID, marginBottom: 60 }}>
          <div>{contentText(content, "copy16", "Product analytics")}</div><div style={{ fontWeight: 700, color: INK }}>{contentText(content, "copy17", "ClickUp 3.0")}</div>
          <div>{contentText(content, "copy18", "Widget brainstorm")}</div><div>{contentText(content, "copy19", "Design system")}</div><div>{contentText(content, "copy20", "Design")}</div>
        </div>
        <div style={{ fontFamily: FONT, fontSize: 110, fontWeight: 750, color: INK }}>{contentText(content, "copy21", "Home")}</div>
        <div style={{ height: 40 }} />
        <div style={{
          ...
          <div style={{ fontFamily: FONT, fontSize: 42, color: MID }}>{contentText(content, "copy22", "Search by app, filetype, or keyword")}</div>
        </div>
        <div style={{ height: 66 }} />
        <div style={{ display: 'flex', gap: 70, fontFamily: FONT, fontSize: 46 }}>
          <div style={{ color: INK, fontWeight: 700 }}>{contentText(content, "copy23", "Recent")}</div>
          <div style={{ color: MID }}>{contentText(content, "copy24", "Favorites")}</div>
        </div>
        {/* ↑ Recent 网格槽位区域留空（卡片从空中落入 GRID_X/GRID_Y 起的两行四列） */}
        <div style={{ height: 560 }} />
        <div style={{ display: 'flex', gap: 70, fontFamily: FONT, fontSize: 44 }}>
          <div style={{ color: INK, fontWeight: 700 }}>{contentText(content, "copy25", "Todo")}</div>
          <div style={{ color: MID }}>{contentText(content, "copy26", "Comments")}</div>
          <div style={{ color: MID }}>{contentText(content, "copy27", "Done")}</div>
          <div style={{ color: MID }}>{contentText(content, "copy28", "Delegated")}</div>
        </div>
        <div style={{ height: 36 }} />
        <div style={{
        ...
        }}>{contentText(content, "copy29", "TODAY")}</div>
        <div style={{ height: 40 }} />
...
const RunwayGroundSkim: React.FC = () => {
