...
const RadialRipplePhoneChips: React.FC = () => {
  const t = useT();
  ...
  return (
    <DesignStage bg="#f2f3f5">
      <div style={{ position: 'absolute', inset: 0, background: '#f2f3f5', overflow: 'hidden' }}>
        ...
        <div style={{ ...chipBase, top: '38%', right: 'calc(50% + 86px)', ...chipStyle(0.22, 0) }}>{contentText(content, "copy0", "Feature one")}</div>
        <div style={{ ...chipBase, top: '56%', left: 'calc(50% + 86px)', ...chipStyle(0.4, 1.8) }}>{contentText(content, "copy1", "Feature detail")}</div>
      </div>
    </DesignStage>
  );
};
return RadialRipplePhoneChips;
}
