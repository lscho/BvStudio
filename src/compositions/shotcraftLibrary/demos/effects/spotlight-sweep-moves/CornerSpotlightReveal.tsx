...
const InboxPanel: React.FC = () => (
  ...
  >
    <div style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
      <div style={{ fontSize: 118, fontWeight: 700, letterSpacing: -2 }}>{contentText(content, "copy0", "Inbox")}</div>
      <div
        style={{
    ...
    <div style={{ display: 'flex', gap: 64, marginTop: 90, fontSize: 44, color: '#555' }}>
      <div style={{ background: '#e9e8f6', color: '#5b55c8', padding: '10px 34px', borderRadius: 14, fontWeight: 600 }}>{contentText(content, "copy1", "All")}</div>
      <div style={{ padding: '10px 0' }}>{contentText(content, "copy2", "Tasks")}</div>
      <div style={{ padding: '10px 0' }}>{contentText(content, "copy3", "Docs")}</div>
      <div style={{ padding: '10px 0' }}>{contentText(content, "copy4", "People")}</div>
      <div style={{ padding: '10px 0' }}>{contentText(content, "copy5", "Chat")}</div>
    </div>
    {[0, 1, 2].map((row) => (
...
const CornerSpotlightReveal: React.FC = () => {
