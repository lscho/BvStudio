...
import { AbsoluteFill, useCurrentFrame, interpolate, Easing, spring, useVideoConfig } from "@/compositions/shotcraftLibrary/runtime";
...
const SegmentedThumbHero: React.FC = () => {
    ...
    <AbsoluteFill style={{ background: G.bg, alignItems: 'center', justifyContent: 'center' }}>
      ...
      >
        ...
        <div style={labelStyle(askActive)}>
          ...
          {contentText(content, "copy0", "Ask")}</div>
        {/* Computer 段 */}
        <div style={labelStyle(!askActive)}>
          <span
            style={{
          ...
          {contentText(content, "copy1", "Computer")}</div>
      </div>
      <ArrowCursor x={curX} y={curY} press={press} />
    </AbsoluteFill>
  );
};
return SegmentedThumbHero;
}
