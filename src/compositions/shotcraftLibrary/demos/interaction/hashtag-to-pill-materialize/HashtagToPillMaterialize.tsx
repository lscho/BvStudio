...
const PILL_W = 740, PILL_H = 236;
const END_SCALE = 0.554;
const SLOT = { x: 361, y: 473 };
...
const Pill: React.FC<{ bg: string; iconColor: string; textColor: string }> = ({ bg, iconColor, textColor }) => (
  <div style={{
    ...
    <span style={{ fontSize: FS, fontWeight: 500, color: textColor, letterSpacing: 2 }}>{contentText(content, "copy0", "music")}</span>
  </div>
);
const HashtagToPillMaterialize: React.FC = () => {
      ...
      {revealed && (
        <>
          <div style={{
            ...
            {contentText(content, "copy1", "My favorite bands")}</div>
          <div style={{
            position: 'absolute', left: 152, top: 618,
            fontSize: 70, fontWeight: 500, color: C.body, lineHeight: 1.33, letterSpacing: 0.3,
          }}>
            {contentText(content, "copy2", "I want to share a few of my favorite bands")}<br />
            {contentText(content, "copy3", "and the song that I always listen when driving")}<br />
            {contentText(content, "copy4", "to home. Welcome. Bring headphones.")}</div>
        </>
      )}
...
return HashtagToPillMaterialize;
}
