...
export function PinBoardCard(props: CompositionRenderProps) {
...
export function ChecklistCard(props: CompositionRenderProps) {
  const legacy = legacyParts(props.text);
...
export function VersusCard(props: CompositionRenderProps) {
  const legacy = legacyParts(props.text);
  ...
  return <div className="os-hud vs" data-theme={paramString(props, "theme", "dark")} style={cardStyle(props.accentColor)}>
    <div className={`vs-side a ${aDim ? "is-dim" : ""}`} style={{ opacity: aProgress, transform: `translateX(${studioLength((1 - aProgress) * -48)})` }}>
    ...
    <div className="vs-badge" style={{ opacity: progressAt(props.timeUs, 500_000, 420_000), transform: `scale(${0.5 + badgeProgress * 0.5})` }}>VS</div>
    <div className={`vs-side b ${bDim ? "is-dim" : ""}`} style={{ opacity: bProgress, transform: `translateX(${studioLength((1 - bProgress) * 48)})` }}>
...
export function EntityChipsCard(props: CompositionRenderProps) {
  const legacy = legacyParts(props.text);
  ...
  return <div className="os-hud etc" data-theme={paramString(props, "theme", "dark")} data-position={paramString(props, "position", "left")} style={cardStyle(props.accentColor)}>
    <div className="etc-row">
      {chips.map((chip, index) => <div className={`etc-chip etc-chip--${chip.style}`} key={`${index}-${chip.name}`} style={{ ...enterStyle(props.timeUs, index * stepUs, 600_000, -30, 0), opacity: progressAt(props.timeUs, index * stepUs, 500_000) }}>
        <div className="etc-name">{chip.name}</div>
        {chip.sub && <div className="etc-sub">{chip.sub}</div>}
      </div>)}
      {(noteA || noteB) && <div className="etc-note" style={{ ...enterStyle(props.timeUs, chips.length * stepUs, 600_000, -20, 0), opacity: progressAt(props.timeUs, chips.length * stepUs, 500_000) }}>
...
export function StatProofCard(props: CompositionRenderProps) {
  const legacy = legacyParts(props.text);
  ...
  const progress = progressAt(props.timeUs, 0, durationUs, "exponential");
  const decimals = Number.isInteger(target) ? 0 : 1;
  const shown = (target * progress).toLocaleString("en-US", { minimumFractionDigits: decimals, maximumFractionDigits: decimals });

  return <div className="os-hud spf" data-theme={paramString(props, "theme", "dark")} data-position={paramString(props, "position", "left")} style={cardStyle(props.accentColor)}>
    <div className="spf-box" style={{ ...enterStyle(props.timeUs, 0, 700_000, 0, 24), opacity: progressAt(props.timeUs, 0, 600_000, "ease") }}>
