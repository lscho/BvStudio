...
describe("React effect registry", () => {
  it("exposes every active effect and its inspector controls", () => {
    ...
    expect(registered.map((entry) => entry.definition.id)).toEqual(allCompositions().filter((effect) => effect.renderer !== "three" && effect.renderer !== "canvas").map((effect) => effect.id));
    const clip = { compositionId: registered[0].definition.id } as CompositionClip;
  ...
  it("maps knowledge effects to dedicated deterministic components", () => {
    ...
    expect(effectControlsFor({ compositionId: "knowledge-causal-chain" } as CompositionClip)[0]).toEqual(expect.objectContaining({ label: "内容（用｜分隔）", rows: 4 }));
  ...
  it("maps talking-head effects to dedicated deterministic components", () => {
    ...
    expect(effectControlsFor({ compositionId: "pin-board" } as CompositionClip)).toEqual(expect.arrayContaining([
  ...
  it("maps all migrated effects to dedicated deterministic components", () => {
    ...
    expect(effectControlsFor({ compositionId: "ring-metric" } as CompositionClip)).toEqual(expect.arrayContaining([
  ...
  it("renders every migrated effect from its persisted defaults", () => {
    for (const id of OVERLAY_STUDIO_EFFECT_IDS) {
      ...
      const first = renderToStaticMarkup(<CompositionContent {...props} />);
  ...
  it("renders timeline-aware persistent effects deterministically", () => {
    ...
    const base = { compositionId: chapter.id, text: chapter.defaultText, color: chapter.defaultColor, accentColor: chapter.defaultAccentColor, fontSize: 48, recipe: chapter.recipe, durationUs: chapter.defaultDurationUs, canvasWidth: 1920, params };
    const opening = renderToStaticMarkup(<CompositionContent {...base} timeUs={2_000_000} />);
    const middle = renderToStaticMarkup(<CompositionContent {...base} timeUs={16_000_000} />);
    expect(opening).not.toBe(middle);
    expect(middle).toBe(renderToStaticMarkup(<CompositionContent {...base} timeUs={16_000_000} />));
  ...
  it("reveals persistent talking points from the playhead without wall-clock animation", () => {
    ...
    const props = {
      compositionId: definition.id, text: "内容框架｜先讲结论｜补充证据｜给出行动", color: "#ffffff", accentColor: "#5fa8ff",
      fontSize: 48, recipe: definition.recipe, durationUs: definition.defaultDurationUs, canvasWidth: 1920
    };
    const early = renderToStaticMarkup(<CompositionContent {...props} timeUs={120_000} />);
    const complete = renderToStaticMarkup(<CompositionContent {...props} timeUs={1_900_000} />);
    expect(early).not.toBe(complete);
    expect(complete).toBe(renderToStaticMarkup(<CompositionContent {...props} timeUs={1_900_000} />));
  ...
  it("renders real odometer reels with all ten digits", () => {
    ...
    const markup = renderToStaticMarkup(<CompositionContent
      compositionId={definition.id} text={definition.defaultText} color={definition.defaultColor} accentColor={definition.defaultAccentColor}
  ...
  it("preserves checklist, versus, entity, and focus-card reference states", () => {
    const render = (compositionId: string, params: Record<string, string | number | boolean>) => {
      const definition = reactEffectDefinition(compositionId).definition;
      return renderToStaticMarkup(<CompositionContent
        compositionId={compositionId} text={definition.defaultText} color={definition.defaultColor} accentColor={definition.defaultAccentColor}
  ...
  it("renders the same knowledge frame for the same virtual time", () => {
    const props = {
      compositionId: "knowledge-myth-fact", text: "越复杂越专业｜清晰比复杂更重要", color: "#ffffff", accentColor: "#ff7b72",
      fontSize: 48, recipe: reactEffectDefinition("knowledge-myth-fact").definition.recipe, durationUs: 4_000_000, canvasWidth: 1920
    };
    const early = renderToStaticMarkup(<CompositionContent {...props} timeUs={0} />);
    const complete = renderToStaticMarkup(<CompositionContent {...props} timeUs={950_000} />);
    expect(early).not.toBe(complete);
    expect(complete).toBe(renderToStaticMarkup(<CompositionContent {...props} timeUs={950_000} />));
  ...
  it("renders a title and multiple quote lines at deterministic staggered times", () => {
    ...
    const props = {
      compositionId: definition.id, text: "关于长期主义｜真正重要的不是走得多快｜而是始终走在正确的方向", color: "#ffffff", accentColor: "#ffb84d",
      fontSize: 48, recipe: definition.recipe, durationUs: definition.defaultDurationUs, canvasWidth: 1920
    };
    const early = renderToStaticMarkup(<CompositionContent {...props} timeUs={350_000} />);
    const complete = renderToStaticMarkup(<CompositionContent {...props} timeUs={1_550_000} />);
    expect(early).not.toBe(complete);
    expect(complete).toBe(renderToStaticMarkup(<CompositionContent {...props} timeUs={1_550_000} />));
