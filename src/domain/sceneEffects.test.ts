...
describe("scene effect matching", () => {
  it("returns stable ids from the active migrated effect library", () => {
    ...
    expect(first.every((id) => allCompositions().some((effect) => effect.id === id))).toBe(true);
    expect(first.every((id) => compositionById(id).kind !== "scene")).toBe(true);
  });
  ...
  it("expands scene selections into atomic editable layers with matching metadata", () => {
    ...
    expect(layers.every((layer) => compositionById(layer.compositionId).kind !== "scene")).toBe(true);
  ...
  it("places ordinary AI-selected effects in semantic canvas regions", () => {
    ...
    expect(suggestedEffectTransform(compositionById("data-bar-chart"), 0).scale).toBeLessThanOrEqual(0.7);
  });
});
