...
describe("project files", () => {
  it("round-trips subtitle themes and migrates version 29 without recoloring existing subtitles", () => {
    ...
    expect(parseProject(JSON.stringify({ ...legacy, schemaVersion: 29 }))).toMatchObject({ schemaVersion: 32, subtitleTheme: { color: "#ffffff", highlightColor: "#ffb84d" } });
  ...
  it("round-trips custom presenter geometry and migrates version 28 presets", () => {
    ...
    expect(parseProject(JSON.stringify(legacy))).toMatchObject({ schemaVersion: 32, presenterSafeArea: { position: "left", widthPercent: 38 } });
  });
  ...
  it("removes transient object URLs and restores a project", () => {
    ...
    expect(parseProject(serialized)).toMatchObject({ schemaVersion: 32, id: project.id, assets: [{ sourcePath: "/source.mp4" }] });
  });
