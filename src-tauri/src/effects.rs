...
use semver::Version;
use std::{collections::{HashMap, HashSet}, f64::consts::PI, fs, path::{Path, PathBuf}};
...
const EXTENDED_EASINGS: [&str; 14] = [
    ...
    "cubic-in", "cubic-out", "cubic-in-out", "quart-out",
    "back-in", "back-out", "back-in-out", "circ-out", "elastic-out", "bounce-out",
];

fn allowed_easings(schema_version: u32) -> &'static [&'static str] {
    if schema_version >= 4 { &EXTENDED_EASINGS } else { &BASE_EASINGS }
}
...
fn validate_chart(chart: &ChartSpecData) -> Result<(), String> {
    if !["counter", "bar", "donut", "line"].contains(&chart.kind.as_str()) { return Err("图表类型无效".into()); }
    let check_series = |values: &Option<Vec<f64>>| {
        values.as_ref().is_none_or(|series| (0..=24).contains(&series.len()) && series.iter().all(|value| finite_range(*value, -1.0e9..=1.0e9)))
    };
    if !check_series(&chart.series) || !check_series(&chart.comparison) { return Err("图表数值超出范围".into()); }
    if chart.categories.as_ref().is_some_and(|items| items.len() > 24 || items.iter().any(|item| item.chars().count() > 20)) {
        return Err("图表分类标签过长".into());
    }
    if let Some(start) = chart.start_value { if !finite_range(start, -1.0e9..=1.0e9) { return Err("图表起始值无效".into()); } }
    if let Some(end) = chart.end_value { if !finite_range(end, -1.0e9..=1.0e9) { return Err("图表结束值无效".into()); } }
    if let Some(max_y) = chart.max_y { if !finite_range(max_y, 1.0e-6..=1.0e9) { return Err("图表纵轴上限无效".into()); } }
    if let Some(decimals) = chart.decimals { if decimals > 4 { return Err("图表小数位过多".into()); } }
    if let Some(grid_lines) = chart.grid_lines { if grid_lines > 5 { return Err("图表网格线过多".into()); } }
    for text in [&chart.prefix, &chart.suffix, &chart.unit] {
        if text.as_ref().is_some_and(|value| value.chars().count() > 8) { return Err("图表前后缀过长".into()); }
    }
    if let Some(duration) = chart.duration_seconds { if !finite_range(duration, 0.05..=30.0) { return Err("图表动画时长无效".into()); } }
    Ok(())
}

fn validate_sound(sound: &SoundDefinitionData) -> Result<(), String> {
    if !safe_id(&sound.id) { return Err("音效 ID 只能包含小写字母、数字、横线和下划线".into()); }
    if sound.name.trim().is_empty() || sound.name.chars().count() > 60 { return Err("音效名称无效".into()); }
    if !(50_000..=3_000_000).contains(&sound.duration_us) { return Err("音效时长必须介于 0.05 到 3 秒".into()); }
    if sound.layers.is_empty() || sound.layers.len() > 8 { return Err("音效必须包含 1 到 8 个合成层".into()); }
    for layer in &sound.layers {
        if !["sine", "triangle", "square", "noise"].contains(&layer.waveform.as_str()) { return Err("音效波形无效".into()); }
        if layer.duration_us < 10_000 || layer.start_offset_us.saturating_add(layer.duration_us) > sound.duration_us { return Err("音效图层时间范围无效".into()); }
        ...
        if layer.attack_us > layer.duration_us || layer.release_us > layer.duration_us { return Err("音效图层包络时长无效".into()); }
    }
    Ok(())
}

fn envelope(layer: &SoundLayerData, elapsed_us: u64) -> f64 {
    let attack = if layer.attack_us == 0 { 1.0 } else { (elapsed_us as f64 / layer.attack_us as f64).min(1.0) };
    let remaining_us = layer.duration_us.saturating_sub(elapsed_us);
    let release = if layer.release_us == 0 { 1.0 } else { (remaining_us as f64 / layer.release_us as f64).min(1.0) };
...
fn synthesize_wav(sound: &SoundDefinitionData) -> Vec<u8> {
    ...
    let mut noise_states = sound.layers.iter().enumerate().map(|(index, _)| (initial_seed ^ (index as u32).wrapping_mul(0x9e37_79b9), 0.0)).collect::<Vec<_>>();
    ...
    for sample_index in 0..sample_count {
        ...
        for (layer_index, layer) in sound.layers.iter().enumerate() {
            if sample_us < layer.start_offset_us || sample_us >= layer.start_offset_us + layer.duration_us { continue; }
            ...
            let phase = 2.0 * PI * (layer.start_frequency_hz * elapsed + 0.5 * frequency_delta * elapsed * elapsed / duration);
...
fn validate_effect(effect: &EffectDefinitionData, schema_version: u32) -> Result<(), String> {
    ...
    if (effect.category == "场景") != (is_scene || is_background) { return Err("场景分类必须声明 sceneLayers 或 sceneBackground".into()); }
    if is_scene && is_background { return Err("组合场景不能同时声明场景背景".into()); }
    if is_scene {
    ...
    if let Some(animation) = &effect.recipe.animation {
        ...
        if schema_version < 4 && uses_v4_features { return Err("图表、扩展缓动与 3D 关键帧需要使用动效包 schemaVersion 4".into()); }
        if !(0.05..=10.0).contains(&animation.duration_seconds) { return Err("关键帧动画时长无效".into()); }
        if !allowed_easings(schema_version).contains(&animation.easing.as_str()) { return Err("关键帧缓动类型无效".into()); }
        ...
        for (index, keyframe) in animation.keyframes.iter().enumerate() {
            ...
            if let Some(name) = &keyframe.easing {
                if !allowed_easings(schema_version).contains(&name.as_str()) { return Err("关键帧缓动类型无效".into()); }
            }
            for tilt in [keyframe.rotate_x, keyframe.rotate_y] {
                if !(-80.0..=80.0).contains(&tilt.unwrap_or(0.0)) { return Err("3D 倾斜角必须介于 -80 到 80 度".into()); }
            }
            if let Some(perspective) = keyframe.perspective {
                if !(200.0..=4000.0).contains(&perspective) { return Err("透视距离必须介于 200 到 4000".into()); }
            }
    ...
    if effect.recipe.animation.is_none() {
        if schema_version < 4 && effect.recipe.chart.is_some() { return Err("图表、扩展缓动与 3D 关键帧需要使用动效包 schemaVersion 4".into()); }
    }
    if let Some(chart) = &effect.recipe.chart { validate_chart(chart)?; }
    if let Some(scene) = &effect.recipe.scene_background {
        if schema_version < 5 { return Err("场景背景需要使用动效包 schemaVersion 5".into()); }
        if !["black-stripes", "white-frame", "dark-grid", "clean-white", "spotlight", "blueprint", "paper-lines", "contrast-side"].contains(&scene.preset.as_str()) {
            return Err("场景背景预设无效".into());
        }
        if !valid_color(&scene.primary_color) || !valid_color(&scene.secondary_color) || !valid_color(&scene.border_color) || !finite_range(scene.intensity, 0.1..=1.0) {
...
fn inspect_contents(contents: &str, path: &Path) -> Result<EffectPackageInfo, String> {
    ...
    if !matches!(package.schema_version, 1 | 2 | 3 | 4 | 5 | 6) { return Err("不支持此动效包版本".into()); }
    ...
    if package.schema_version < 6 && (!package.sounds.is_empty() || package.effects.iter().any(|effect| !effect.sound_cues.is_empty())) {
        return Err("声明式音效需要使用动效包 schemaVersion 6".into());
    }
    if package.sounds.len() > 32 { return Err("动效包最多包含 32 个音效".into()); }
    let mut sound_ids = HashSet::new();
    for sound in &package.sounds {
        validate_sound(sound)?;
        if !sound_ids.insert(sound.id.as_str()) { return Err(format!("音效 ID 重复: {}", sound.id)); }
    }
    ...
    for effect in &package.effects {
        ...
        for cue in &effect.sound_cues {
            if !sound_ids.contains(cue.sound_id.as_str()) { return Err(format!("动效引用了包内不存在的音效: {}", cue.sound_id)); }
            if cue.offset_us >= effect.default_duration_us || !finite_range(cue.volume, 0.0..=1.0) { return Err("动效音效触发参数无效".into()); }
        }
    ...
    let sound_durations = package.sounds.iter().map(|sound| (sound.id.clone(), sound.duration_us)).collect::<HashMap<_, _>>();
    ...
    for effect in &mut effects {
        ...
        for cue in &mut effect.sound_cues {
            cue.duration_us = sound_durations.get(&cue.sound_id).copied().unwrap_or_default();
...
fn sound_cache_dir(app: &AppHandle) -> Result<PathBuf, String> {
    let path = app.path().app_data_dir().map_err(|error| error.to_string())?.join("audio").join("effect-sounds");
...
fn materialize_sounds(app: &AppHandle, info: &mut EffectPackageInfo) -> Result<(), String> {
    if info.sounds.is_empty() { return Ok(()); }
    ...
    for sound in &info.sounds {
        ...
        let suffix = digest.iter().take(8).map(|byte| format!("{byte:02x}")).collect::<String>();
        ...
        let cache_valid = fs::metadata(&path).map(|metadata| metadata.is_file() && metadata.len() == expected_size).unwrap_or(false);
        if !cache_valid {
            fs::write(&path, synthesize_wav(sound)).map_err(|error| format!("无法生成动效音效: {error}"))?;
        }
        paths.insert(format!("{}:{}", info.manifest.id, sound.id), path.to_string_lossy().into_owned());
    }
...
mod tests {
    ...
    fn validates_the_example_package_and_install_version_policy() {
        ...
        assert_eq!(info.effects[0].sound_cues[0].sound_id, "bvideo-starter:whoosh-short");
    ...
    fn rejects_v6_sound_cues_that_reference_missing_sounds() {
        let contents = include_str!("../../examples/effects/starter-pack.bveffect").replace("\"soundId\": \"whoosh-short\"", "\"soundId\": \"missing\"");
        assert!(inspect_contents(&contents, Path::new("invalid.bveffect")).err().unwrap().contains("不存在的音效"));
    }
    ...
    fn validates_v4_charts_and_extended_easings_but_rejects_them_below_v4() {
        ...
        assert_eq!(info.effects[0].recipe.animation.as_ref().unwrap().keyframes[1].easing.as_deref(), Some("ease-out"));

        let downgraded = base.replace("\"schemaVersion\":4", "\"schemaVersion\":3");
        assert!(inspect_contents(&downgraded, Path::new("old.bveffect")).err().unwrap().contains("schemaVersion 4"));

        let bad_kind = base.replace("\"kind\":\"bar\"", "\"kind\":\"hologram\"");
        assert!(inspect_contents(&bad_kind, Path::new("bad.bveffect")).err().unwrap().contains("图表类型无效"));
    }
    ...
    fn validates_v5_scene_backgrounds_and_rejects_invalid_intensity() {
        ...
        assert_eq!(info.effects[0].recipe.scene_background.as_ref().unwrap().preset, "black-stripes");

        let downgraded = base.replace("\"schemaVersion\":5", "\"schemaVersion\":4");
        assert!(inspect_contents(&downgraded, Path::new("old.bveffect")).err().unwrap().contains("schemaVersion 5"));
        let invalid = base.replace("\"intensity\":0.72", "\"intensity\":2");
        assert!(inspect_contents(&invalid, Path::new("invalid.bveffect")).err().unwrap().contains("参数无效"));
    }
