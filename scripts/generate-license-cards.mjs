...
import { CARD_KEY_CHARSET, CARD_KEY_RE, sha256Hex } from "../edge/license-core.mjs";
...
/**
 * 生成卡密清单。plan: "lifetime"（永久）| "period"（按天数，需 days）。
 */
export async function generateCards({ count, plan, days = null, issuedAt = Date.now(), randomSource = randomBytes }) {
  ...
  if (plan !== "lifetime" && plan !== "period") {
    throw new Error('plan 必须是 "lifetime" 或 "period"');
  }
  ...
  return { generatedAt: new Date(issuedAt).toISOString(), plan, days: plan === "period" ? days : null, cards };
}
