...
- Shotcraft 首轮：在「动效 → 展示」中搜索 `Shotcraft`，添加柔焦标题、前后对比、焦点巡览、空间步进、聚光悬浮卡、叠卡扇展。支持替换截图、修改文案、点击定位焦点与独立增加停留；同轨相邻整屏镜头可用「流白」「整屏上推」衔接。预览与逐帧导出共用实现，首轮手动编排，暂不参与 AI 字幕动效匹配。用法、素材数量及转场边界见 [Shotcraft 镜头](docs/shotcraft.md)。
