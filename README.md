...
- 字幕与分镜共用 AI 编排：从字幕「匹配动效」或顶部「AI 镜头编排 → 按现有字幕编排」进入，支持自动混合、纯 A-roll、纯 B-roll 和带工程时间的分镜要求（如 `0-5秒 A-roll；5-12秒 B-roll 用产品截图对比`）。两阶段共用字幕、素材名称/角色/关联字幕摘要和已有镜头内容；角色与时间范围、全屏覆盖、真实素材槽、文案及高亮词会校验，Shotcraft 使用同一字幕组时长并保留来源字幕。匹配记录可查看角色、字幕范围与选用动效。纯 A-roll 需要已放入口播视频；纯 B-roll 保留原音轨。该入口不上传图片识别，语义选材仍需审片；需人工圈焦点的镜头继续使用图片编排入口。详见 [字幕分镜关联](docs/storyboard-linking.md)。
