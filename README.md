...
- AI 内容支持顺序插入、替换区间和叠加区间；内容生成只写入文案、口播与时间字幕，用户随后可独立调用动效匹配。匹配会先把连续 2–8 条字幕组织为约 6–15 秒的场景，同场景最多逐步累积 4 个文字或图表层并保持到场景结束；要点钉板、步骤时间线和步骤清单的递增状态会合并为一个覆盖场景的动效，由组件内部逐项显现。导入视频的现有角色和关联字幕摘要会参与 A-roll/B-roll 判断，信息密集场景可选择一段静音全屏 B-roll，主口播 A-roll 则继续播放并只追加克制运镜。模型未正确分组时客户端会按字幕时间连续性自动兜底，写入前再由本地布局解析器避开底部字幕、章节进度和已有动效，规则见 [`docs/ai-motion-layout.md`](docs/ai-motion-layout.md)；
