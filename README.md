...
- 内置动效库仅保留从 Overlay Studio 迁入的 20 个口播动效；旧版文字、图表和组合场景定义仅在打开已有工程时兼容解析，不再出现在动效库、本地检索或 AI 候选中；
...
- 动效元数据集中在 `src/domain/effects.ts`，React 卡片注册与参数控件集中在 `src/effects/registry.tsx`；预览和导出复用同一组件树，并由整数微秒播放头计算确定性帧。Overlay Studio 的 20 个口播动效包括：金句定格、步骤时间线、数据排名条、金句强调条、术语解释卡、步骤清单、3D 终端、环形指标、双栏对比、界面标注、排版重组、模糊浮现、翻牌计数器、人物聚焦卡、章节导航条、双语字幕轨、数字实证、增长曲线、人物机构名牌和要点钉板。各组件的专属参数随工程文件保存；结构化内容使用 `｜`、`|` 或换行分隔，模板按播放头逐项显现并保持在场。“动效”Tab 顶部可从 6 个预设中提前选择统一主题色，后续手动添加和 AI 匹配的动效与字幕高亮都会采用该颜色；项目级 `skin / style / font` 和完整语义色仍可在画布设置中调整，单个动效也可切换为独立文字色、强调色和背景底色；
...
- 资源面板内置片头冲击、快切甩镜、字幕弹出、重点重音、高光闪亮、悬念上升和片尾收束等自媒体剪辑音效，可试听并在播放头手动加入音效轨；AI 场景匹配也会从同一白名单中按字幕语义稀疏选择，连续场景最多一个且相邻音效至少间隔 2.5 秒。内置立体声 WAV 在使用时生成，桌面端缓存后供预览与 FFmpeg 导出共用；
