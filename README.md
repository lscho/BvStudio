...
- 动效元数据集中在 `src/domain/effects.ts`，React 卡片注册与参数控件集中在 `src/effects/registry.tsx`；预览和导出复用同一组件树，并由整数微秒播放头计算确定性帧。内置概念图谱、因果链、论点证据板、误区纠偏和金句逐行等知识表达卡片，结构化内容使用 `｜` 或换行分隔。“动效”Tab 顶部可从 6 个预设中提前选择统一主题色，后续手动添加和 AI 匹配的动效、场景边框与字幕高亮都会采用该颜色；项目级 `skin / style / font` 和完整语义色仍可在画布设置中调整，单个动效也可切换为独立文字色、强调色和背景底色；
